...
def main():
    ...
    p = subprocess.Popen(['sinfo','-O','nodehost,gresused:28'],stdout=subprocess.PIPE,
    ...
    itemno = 0
    for line in outstr:
        if "compute" in line or "argon" in line or "aquila" in line or "chen" in line or "hetao" in line or "login" in line:
            continue
        line = re.sub('\"', '', line)
        if itemno == 0:
            # print line
            ...
            if match:
                ...
                namestr = n0+dd+n1
                # print namestr
                gpuinfo = re.split(r"\:",temp1[1])
                gpuno = re.split(r"\(",gpuinfo[2])[0]
                gpuidx = gpuinfo[3][:-1]
                nodedict[namestr] = temp1[0]+'\t\t\t'+gpuinfo[1]+":"+gpuno+"("+gpuidx+")"
